...
handle_error_report(_, _, crash_report, [Report, Neighbors], S) ->
    ...
    %% Do we need that or are all Errors like this?!?
    {Reason1, Trace1} = case is_list(Reason) of
                            true ->
                                {proplists:get_value(reason, Reason),
                                 proplists:get_value(stacktrace, Reason)};
                            _ ->
                                {Reason, Trace}
                        end,
    notify(Type, {Reason1, Trace1},
